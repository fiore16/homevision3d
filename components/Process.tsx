    ...
    <section id="process" className="bg-black">
      ...
      {steps.map(({ number, title, body, image }, i) => (
        ...
        >
          ...
          {/* Text — 40% */}
          <div className={`md:w-2/5 flex flex-col justify-center px-10 md:px-16 py-16 bg-black border-t md:border-t-0 ${
            i % 2 === 0 ? 'md:border-l' : 'md:border-r'
          } border-white/5`}>
            <span className="font-display text-7xl font-black text-white/5 leading-none mb-4 select-none">
              {number}
            </span>
            <h3 className="font-display text-2xl md:text-3xl font-bold text-white mb-4 -mt-8">
              {title}
            </h3>
